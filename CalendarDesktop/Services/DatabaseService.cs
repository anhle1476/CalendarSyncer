...
namespace CalendarDesktop.Services
{
    ...
    public class DatabaseService : IDatabaseService
    {
        ...
        /// <summary>
        /// Gets all calendar events from the database.
        /// </summary>
        /// <returns>A list of all calendar events.</returns>
        public async Task<List<CalendarEvent>> GetAllEventsAsync()
        {
            const string sql = "SELECT * FROM CalendarEvents ORDER BY StartTime DESC;";
            using (var connection = new SqlConnection(_connectionString))
            {
                var events = await connection.QueryAsync<CalendarEvent>(sql);
